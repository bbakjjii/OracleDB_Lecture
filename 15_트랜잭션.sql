...
/*
    # 트랜잭션 (Transaction)
    - 하나의 작업 단위 (ex : 송금)
    - 트랜잭션에 속한 모든 작업이 정상적으로 완료되는 경우에만 변경사항을 확정 지을 수 있는 기능을 제공한다 (All or Nothing)
    ...
    - 하나의 트랜잭션은 마지막으로 실행된 커밋으로부터 새로운 커밋을 실행하기 전까지 수행되는 모든 DML만을 의미한다
     (INSERT, UPDATE, DELETE 등의 테이블 데이터 수정은 트랜잭션 관리의 대상이지만, CREATE TABLE 등은 트랜잭션 관리의 대상이 아니다)
*/
SELECT * FROM fruits;
